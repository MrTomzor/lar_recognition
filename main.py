...
def getContours(color, img_hsv):
    ...
    mask = cv2.inRange(img_hsv, np.array([color - hueThreshold, 150, 50]), np.array([color + hueThreshold, 255, 255]))
    print(mask)

    imageio.imwrite("mask.png", mask)
    img_map, contours, hierarchy = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)    
